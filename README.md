...
## 🎨 Recent Studio UI & Lightbox Updates
 ...
 - **Persistent Card Footers & Copy Prompt**: Redesigned the gallery item card to display prompts and actions in a persistent footer directly below the media instead of on-hover overlay. Replaced the hover state with a full-card elevation transition. Included "Add Image Ref", "Add Video Ref", and a new "Copy Prompt" action button side-by-side using flexbox grids, color-coded for active context (crimson for references, slate gray for copy action).
